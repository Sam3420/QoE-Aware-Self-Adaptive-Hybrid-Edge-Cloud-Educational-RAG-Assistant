...
class RetrievalQualityService:
    ...
    def evaluate(self, *, question: str, hits: list[dict]) -> RetrievalQualityDecision:
        if not hits:
            return RetrievalQualityDecision(
                is_adequate=False,
                reason="retrieval_quality_insufficient",
                top_score=None,
                query=question,
            )
        ...
        if best_score >= self.min_score:
            return RetrievalQualityDecision(
                is_adequate=True,
                reason="retrieval_quality_adequate",
    ...
    @staticmethod
    def corrective_query(question: str) -> str:
        return f"{question.strip()} key concepts definition explanation evidence"
